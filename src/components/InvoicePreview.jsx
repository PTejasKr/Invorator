...
export default function InvoicePreview({ data, lang = "en", currency = "INR" }) {
  ...
  const renderItems = items.map((item, idx) => {
    ...
    const itemTax = parseFloat(item.taxRate ?? taxRate ?? 18) || 0;
    const gstRate = itemTax > 0 ? itemTax : 0;
              ...
              <td colSpan="4" className="p-0">
                <table className="gst-calc-table">
                  <tbody>
                    ...
                    {shippingCharges > 0 && (
                      <tr>
                        <td className="text-right pr-3">Add : Delivery Charges</td>
                        <td className="text-right"><strong>{formatCurrency(shippingCharges)}</strong></td>
                      </tr>
                    )}
                    {freightCharges > 0 && (
                      <tr>
                        <td className="text-right pr-3">Add : Freight Charges</td>
                        <td className="text-right"><strong>{formatCurrency(freightCharges)}</strong></td>
                      </tr>
                    )}
                    <tr className="border-top-thick">
